...
abstract class WP_Incluyeme_Countries_Abs
{
    ...
    public static function updateUsersInformation($city, $dateBirthDay, $fPhone, $fiPhone, $genre, $mPhone, $state, $street, $phone, $meetingIncluyeme = false)
    {
    ...
    public static function updatePrefersJobs($userID, $preferJobs)
    {
        ...
        error_log(print_r($myrows, true));
        foreach ($myrows as $details) {
            update_user_meta(get_current_user_id(), 'area_interes', $details->jobs_prefers);
            update_user_meta(get_current_user_id(), 'area', $details->jobs_prefers);
            error_log(print_r(get_user_meta(get_current_user_id(), 'area_interes'), true));
        }
    ...
    public static function updateIdioms($userID, $idioms, $oLevel, $wLevel, $sLevel, $idiomsOther)
    {
        ...
        for ($i = 0; $i < count($idioms); $i++) {
            ...
            if ($idiomsName === self::$idioma_ingles) {
                $user_id = self::$wp->get_results('SELECT * from ' . self::$wp->prefix . 'wpjb_resume where id = ' . $userID);
                if (count($user_id) > 0) {
                    update_user_meta($user_id[0]->user_id, 'english_level', $level);
                } else {
                    error_log(print_r('User not found' . $user_id, true));
                }
            }
            if ($idiomsName !== null) {
...
FROM wp_incluyeme_users_dicapselect where resume_id ={$id}");
        ...
        return (object)[
            'discapsSelected' => $discapsSelected,
            'assets' => self::getCV($id),
            'idioms' => $idioms,
            'discap' => $discaps,
            'work' => $works,
            'education' => $education,
            'information' => $information[0],
            'name' => get_user_meta(get_current_user_id(), 'first_name')[0],
            'last_name' => get_user_meta(get_current_user_id(), 'last_name')[0]
        ];
    }
    ...
    public static function getUser($id, $application = null)
    {
...
GROUP BY {$prefixSearch}incluyeme_users_questions.question_id,
        ...
        return (object)[
            ...
            'education' => $education,
            'information' => $information[0],
            'name' => get_user_meta($userID[0]->user_id, 'first_name'),
            'last_name' => get_user_meta($userID[0]->user_id, 'last_name'),
            'tagsIncluyeme' => get_user_meta($userID[0]->user_id, 'tagsIncluyeme'),
            'comments' => $application ? get_option('applicationComment_' . $application) : ''
        ];
    }
    ...
    {
        $userID = self::$wp->get_results('SELECT * from ' . self::$wp->prefix . 'wpjb_resume where id = ' . $id);
        update_user_meta($userID[0]->user_id, 'tagsIncluyeme', $tags);
        
        return '';
    }
    
    public static function updateComments($id, $comment): string
    {
        update_option('applicationComment_' . $id, $comment);
