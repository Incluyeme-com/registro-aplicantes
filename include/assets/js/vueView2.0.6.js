 ...
 */
// Espera a que se cargue completamente el documento antes de ejecutar el código JavaScript
Vue.config.ignoredElements = ['x-incluyeme', 'fb:login-button']
let app = new Vue({
    el: '#incluyeme-login-wpjb',
    data: {
        userID: null,
        checkBoxesInpu: false,
        dateStudiesDLaboral: [],
        dateStudiesHLaboral: [],
        url: null,
        countries: [],
        ...
        myIMG: null,
        tagsData: [
            {id: "DTF", label: "#DTF", selected: false},
            {id: "TopList", label: "#TopList", selected: false},
            {id: "Academias", label: "#Academias", selected: false},
            {id: "SoftSkills", label: "#SoftSkills", selected: false},
            {id: "labs", label: "#Laboratorios", selected: false},
            {id: "Migrante", label: "#Migrante", selected: false},
            {id: "Pasantia", label: "#Pasantia", selected: false},
    ...
    methods: {
        editTagsSave: async function () {
            const selectedTags = this.tagsData.filter(tag => tag.selected);
            const data = {
                selectedTags: JSON.stringify(selectedTags),
                userID: this.userID
            }
            axios.post(this.url + '/incluyeme-login-extension/include/verifications/viewResume.php', data)
        ...
        saveComments: async function () {
            ...
            const data = {
                ...
                commentsNews: this.comments
            }
            iziToast.info({
                title: 'Por favor, espere',
                message: 'Estamos guardando su informacion.',
        ...
        editTags() {
            this.checkBoxesInpu = !this.checkBoxesInpu;
        },
        setID: async function (userID, url, applicationId) {
            this.url = url;
            this.userID = userID;
            ...
            let data = await jQuery.ajax({
                url: this.url + '/incluyeme-login-extension/include/verifications/viewResume.php?user=' + userID + '&applicationId=' + applicationId,
                type: 'GET',
                dataType: 'json'
            }).done(success => {
                return success;
            ...
            if (data.message.tagsIncluyeme.length) {
                const tags = JSON.parse(data.message.tagsIncluyeme[0])
                for (const element of tags) {
                    let datoJSON = element;
                    let objetoEnTagsData = this.tagsData.find(function (tag) {
                        return tag.id === datoJSON.id;
                    });

                    if (objetoEnTagsData) {
                        objetoEnTagsData.selected = datoJSON.selected;
                    }
                }
            }
            ...
            jQuery("#incluyeme-login-textes :input").prop("disabled", true);
        },
